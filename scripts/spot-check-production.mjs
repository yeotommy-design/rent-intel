...
const defaultBaseUrl = "https://rent-intel.com";

const requiredQueries = [
  { query: "Thomson Plaza", expectedTitle: "Thomson Plaza retail" },
  { query: "Palais Renaissance", expectedTitle: "Palais Renaissance retail" },
  { query: "Seah Street", expectedTitle: "Seah Street retail" },
  { query: "Liat Towers", expectedTitle: "Liat Towers retail" },
  { query: "Cuppage annexe", expectedTitle: "Cuppage Rise annexe retail" },
  { query: "Cuppage Rise annexe retail", expectedTitle: "Cuppage Rise annexe retail" },
  { query: "Chinatown shophouse retail", expectedTitle: "Chinatown shophouse retail" },
  { query: "Tampines Mall retail", expectedTitle: "Tampines Mall retail" }
];
  ...
  return function findRecord(query) {
    ...
    const familyAliasMatch = activeRecords
      .map((record) => {
        ...
        return { record, titleStarts, aliasStarts, extraWordPenalty };
      })
      .filter(Boolean)
      .sort((a, b) => {
...
function extractAssetUrl(html, regex, baseUrl, fallbackPath) {
  const match = html.match(regex);
  const assetPath = match?.[1] || fallbackPath;
  return toAbsoluteUrl(baseUrl, assetPath);
}

async function main() {
  const baseUrlArg = process.argv.find((arg) => arg.startsWith("--url="));
  const baseUrl = (baseUrlArg ? baseUrlArg.split("=").slice(1).join("=") : defaultBaseUrl).replace(/\/$/, "");
  const homepageUrl = `${baseUrl}/`;
  const homepageHtml = await fetchText(homepageUrl);
  const appAssetUrl = extractAssetUrl(homepageHtml, /<script\s+src="([^"]*app\.js[^"]*)"><\/script>/i, baseUrl, "./app.js");
  const dataJsAssetUrl = extractAssetUrl(homepageHtml, /<script\s+src="([^"]*rentintel-sample-data\.js[^"]*)"><\/script>/i, baseUrl, "./data/rentintel-sample-data.js");
  const dataJsonUrl = extractAssetUrl(homepageHtml, /<script\s+src="([^"]*rentintel-sample-data\.js[^"]*)"><\/script>/i, baseUrl, "./data/rentintel-sample-data.js")
    .replace(/rentintel-sample-data\.js(?:\?.*)?$/i, "rentintel-sample-data.json");

  const payload = await fetchJson(dataJsonUrl);
  const findRecord = findRecordFactory(payload.records || []);

  const results = requiredQueries.map(({ query, expectedTitle }) => {
    const resolved = findRecord(query);
    return {
      query,
      expectedTitle,
      gotTitle: resolved?.title || null,
      gotId: resolved?.id || null,
      passed: resolved?.title === expectedTitle
    };
  });

  const failures = results.filter((item) => !item.passed);
  console.log("RentIntel production spot-check");
  console.log(JSON.stringify({
    ...
    datasetUpdatedAt: payload.updatedAt,
    checks: results.length,
    failures: failures.length
  }, null, 2));
  results.forEach((item) => {
    console.log(`${item.passed ? "PASS" : "FAIL"} ${item.query} -> ${item.gotTitle || "null"} (expected ${item.expectedTitle})`);
  });
